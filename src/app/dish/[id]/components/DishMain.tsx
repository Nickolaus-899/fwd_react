...
function DishMain({ params }: { params: { id: string } }) {
  ...
  const [showAddingToMenuForm, setShowAddingToMenuForm] = useState(false);
  ...
  const addDishToMenuHandler = () => {
    addDish(client, details).then((clients) => {
      for (let i = 0; i < clients.length; i++) {
        if (clients[i].token === userTokenName) {
          setClient(clients[i]);
        }
  ...
  return (
    <div>
      ...
      <div className="DishTitle">
        {details.title}
        {details.vegetarian ? <LuVegan className="DishTitleIcons" /> : null}
      </div>
      ...
      <div className="DishDetailsWrapper">
        ...
        <li className="DishCharsList">
          <div className="SingleDetailWrapper">
            <div className="ItemInDetail">Calories: {details.calories}</div>
          </div>
          <div className="SingleDetailWrapper">
